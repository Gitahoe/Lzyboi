...
# Usage: lboi -d [PATH]
set_DP_directory() { 
	if [ -z $1 ]; then path=$(cd ${SCRIPT_DIR}/datadumps_default/ && pwd)
	else path=$(cd "$1" 2>/dev/null && pwd)
...
DP_new() {
    ...
    mkdir "$DP_DIR/$DIRNAME"; touch "$DP_DIR/$DIRNAME/settings.sh"; echo "New datadump created: /$DIRNAME"
}
...
# Shows the target data that is currently set for this datadump.
# Shows the (ORGANISED) latest output of a command that was executed while the datadump was active.
# Basically the contents of the datadump
#
# Usage: lboi -t
 show_target() {
	echo "T_HOST"
